...
from .gates import apply_sheet, build_sheet, days_since_last, latest_sheet
from .pipeline import Pipeline
from .projects import create_project, drop_project, load_projects, restore_project, write_views
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "material":
            return _material(load_config(args.vault), args.decision, args.source)
        if args.command == "review":
            return _review(load_config(args.vault), args.since)
        if args.command == "apply":
            return _apply(load_config(args.vault), args.sheet)
        if args.command == "drop":
            return _drop(load_config(args.vault), args.project_id)
        if args.command == "restore":
            return _restore(load_config(args.vault), args.project_id)
...
def _material(config: Config, decision: str | None, source: str | None) -> int:
...
def _review(config: Config, since: str | None, today: date | None = None) -> int:
...
def _apply(config: Config, sheet_date: str | None, today: date | None = None) -> int:
...
def _drop(config: Config, project_id: str) -> int:
...
def _restore(config: Config, project_id: str) -> int:
