...
- **Gate 1 is a sheet of its own, not a section of the weekly digest.** The
  ...
  not tied to one period and must survive rebuilding. `review/<date>.md` is
...
- The session was called *triage* while it was being written; the word
  ...
  directory and state machine are all `review`.
...
## Step 5 — Gate 1: `review` and `apply`

Design:

- `asterism review [--since DATE]` writes `review/<date>-<time>.md`: every
