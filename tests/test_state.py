...
class StateBackendContract:
    ...
    def test_digest_state_round_trip(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            backend = self.backend_type(Path(temporary) / self.filename)
            digest = DigestState("day", "2026-09-21", "2026-09-21", "digest/daily/2026/2026-09-21.md", "open", "2026-09-21T10:00:00+08:00")
            backend.save_digest(digest)
            self.assertEqual(digest, backend.get_digest("day", "2026-09-21"))
            backend.save_digest(dataclasses.replace(digest, state="closed", generated_at="2026-09-22T00:00:00+08:00"))
            self.assertEqual("closed", backend.get_digest("day", "2026-09-21").state)
            backend.save_digest(DigestState("week", "2026-09-18", "2026-09-24", "digest/weekly/2026/2026-W39.md", "open", "x"))
            self.assertEqual(["day", "week"], [d.level for d in backend.digests()])
    ...
    def test_migrates_schema_1(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            path.write_text(json.dumps({
                "schema_version": 1,
                "sources": {"flomo": {"m1": {
                    "source": "flomo", "source_id": "m1", "relative_path": "notes/flomo/m1.md",
...
class SQLiteStateTest(StateBackendContract, unittest.TestCase):
    ...
    def test_migrates_schema_1(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            connection.execute(
                "INSERT INTO item_state VALUES ('flomo', 'm1', 'notes/flomo/m1.md', 'sha256:x', NULL, '2026-09-01T00:00:00+00:00')"
            )
            connection.commit()
            connection.close()
